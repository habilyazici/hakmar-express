...
/**
 * These three lines are the whole point of @hakmar/contracts.
 *
 * Each asserts that an enum here and the union the web consumes describe the
 * same set of strings, in both directions. Add a metric to one side only and
 * the build fails, naming the type — instead of the web offering a dropdown
 * option the API answers with a 400, which is how the two drifted apart
 * before there was anything joining them.
 */
